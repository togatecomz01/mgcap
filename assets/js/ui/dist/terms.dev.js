  ...
  var TermsAgreement = function () {
    ...
    return {
      ...
      toggleAll: function toggleAll(isChecked) {
        $allChks.prop('checked', isChecked); // this.uptSubmitBtn();
...
$(function () {
  ...
  var mq = window.matchMedia('(max-width: ' + BREAKPOINT + 'px)'); // 개별 요소에 대해 백업/복원/제거
  ...
  function toMobile($scope) {
    var $els = ($scope || $(document)).find('label[data-open-bak]'); // 백업이 있는 애들은 모두 복원
  ...
  function toPC($scope) {
    var $els = ($scope || $(document)).find('label[data-popup-open], label[data-open-bak]'); // 우선 새로 들어온 요소도 백업 보장

    backupIfNeeded($els); // 그리고 data-popup-open 제거

    $els.filter('[data-popup-open]').removeAttr('data-popup-open');
  } // 초기 백업


  backupIfNeeded($('label[data-popup-open]')); // 모드 적용

  function applyMode(isMobile, $scope) {
    if (isMobile) toMobile($scope);else toPC($scope);
  } // 최초 1회


  applyMode(mq.matches); // 브레이크포인트 변경 감지
