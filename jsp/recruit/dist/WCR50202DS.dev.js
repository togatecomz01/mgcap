...
$(function () {
  ...
  function toggleReadonlyAll(lock) {
    ...
    if (lock) {
      ...
      $checks.prop('disabled', false).removeAttr('aria-disabled').off('click._lock'); // ���������� "����"�� ��� ���� select�� disabled ����

      var $milType = $('#milType');
      var $milBranch = $('#milBranch');

      if ($milType.length && $milBranch.length && $milType.val() === 'exempt') {
